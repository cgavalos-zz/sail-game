    ...
    elements (_elements)
{
    ...
    float h = 1.0f; // y
    mass = 100.0f * w * d * h;
    float Ic1 = (1.0f / 12.0f) * mass * (d * d + h * h);     // x
    float Ic2 = (1.0f / 12.0f) * mass * (w * w + d * d);     // y
    float Ic3 = (1.0f / 12.0f) * mass * (w * w + h * h);     // z
    I = glm::mat3 (1.0f);
    I[0][0] = Ic1;
    I[1][1] = Ic2;
    I[2][2] = Ic3;
...
void RigidObjectPhysicsComponent::update (WaterSurface const & waterSurface, float timeStep)
{
    ...
    std::vector<PointForce> buoyantForces = waterSurface.buoyantForcesOnMesh(mesh, glm::vec3(0.0f, 1.0f, 0.0f), 101.325e3);
    ...
    // Rotation Friction
    float lOmega = glm::length (omega);
    glm::vec3 dDrag = -omega / lOmega;
    float lengthScale = 10.0f;
    float areaScale = lengthScale * lengthScale;
    float vAnalogue = lOmega * lengthScale;
    float q = 0.5f * rho_water * vAnalogue * vAnalogue;
    float factor = 1.0f;
    float Cd = 0.4f;
    float magn = factor * lengthScale * q * Cd * areaScale;
...
void
RigidObjectPhysicsComponent::pointForces (
    ...
    float timeStep)
{
    ...
    {
        PointForce cur = pfs.at (i);
        glm::vec3 forceV = cur.getMagnitude() * cur.getDirection();
        forceSum += forceV;
        glm::vec3 r = cur.getPoint() - position;
        glm::vec3 M = glm::cross (r, forceV);
        momentSum += M;
    }
...
{
    glm::vec3 forceV = pf.getDirection() * pf.getMagnitude();
    force (glm::mat3 (rotM) * forceV, timeStep);
    glm::vec3 r = pf.getPoint() - position;
    glm::vec3 M = glm::cross (r, forceV);
    moment (M, timeStep);
}
...
{
    // solve for alpha
    glm::vec3 alpha = invI * (M - glm::cross (omega, I * omega));
    // apply alpha
...
{
    float length = glm::length (omega);

    if (length > 0)
    {
        glm::vec3 dirOmega = omega / length;
        float dOmega = length * timeStep;
        glm::mat3 metaRot;
